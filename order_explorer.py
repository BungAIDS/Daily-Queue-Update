...
def taskbar_ps1_text(html_name: str = HTML_NAME) -> str:
    ...
    lines = [
        ...
        "# so keep a per-user local copy of the .ico and reference that.",
        ...
        '$icon = $localIcon + ",0"',
        f"$relaunch = 'wscript.exe \"' + (Join-Path $here \"{LAUNCH_VBS_NAME}\") + '\"'",
        "",
        "$src = @'",
        *csharp,
        "'@",
...
def launch_vbs_text(html_name: str = HTML_NAME) -> str:
    ...
    flash) and runs the taskbar-identity stamper. This is also what a pinned
    taskbar button relaunches. Pure ASCII + CRLF."""
    lines = [
        ...
        'Set shell = CreateObject("WScript.Shell")',
        "here = fso.GetParentFolderName(WScript.ScriptFullName)",
        f'page = fso.BuildPath(here, "{html_name}")',
...
def write_explorer(payload: Dict[str, Any], out: Path | None = None) -> Path:
    ...
    for helper_name, helper_text in (
        (LAUNCH_VBS_NAME, launch_vbs_text(out.name)),
