...
LAUNCH_VBS_NAME = "Open GL Queue Explorer.vbs"
TASKBAR_PS1_NAME = "glq_taskbar.ps1"
# The window's own Windows taskbar identity. An Edge --app window normally
# keeps Edge's AppUserModelID, so the taskbar groups it under Edge and shows
# the Edge icon no matter what the favicon or any .lnk icon says. Stamping
# this ID (plus the Relaunch* properties) on the window is the same
# mechanism Chromium itself uses to give browser profiles separate taskbar
# icons — it is the only thing that changes the icon of a RUNNING window.
APP_USER_MODEL_ID = "CBCInsider.GLQueueExplorer"
...
def shortcut_vbs_text(html_name: str = HTML_NAME, ico_name: str = ICON_ICO_NAME) -> str:
    """Create a Desktop .lnk that opens the Explorer through the silent VBS
    opener — which starts the app window AND stamps its taskbar identity, so
    both the shortcut and the running window's taskbar button show the fan
    icon. Targeting the browser directly is exactly the old attempt that
    could not work: a .lnk icon never changes a running window's button."""
    del html_name  # the opener resolves the page itself
    lines = [
        "' Creates a Desktop shortcut for GL Queue Explorer with a custom icon.",
        "' Auto-generated by order_explorer.py next to the Explorer page.",
        "Option Explicit",
        "Dim fso, shell, here, launcher, iconPath, desktop, lnk",
        "Set fso = CreateObject(\"Scripting.FileSystemObject\")",
        "Set shell = CreateObject(\"WScript.Shell\")",
        "here = fso.GetParentFolderName(WScript.ScriptFullName)",
        "launcher = fso.BuildPath(here, \"" + LAUNCH_VBS_NAME + "\")",
        "iconPath = fso.BuildPath(here, \"" + ico_name + "\")",
        "desktop = shell.SpecialFolders(\"Desktop\")",
        "Set lnk = shell.CreateShortcut(fso.BuildPath(desktop, \"GL Queue Explorer.lnk\"))",
        "lnk.TargetPath = \"wscript.exe\"",
        "lnk.Arguments = \"\"\"\" & launcher & \"\"\"\"",
        "lnk.WorkingDirectory = here",
        "If fso.FileExists(iconPath) Then lnk.IconLocation = iconPath & \",0\"",
        "lnk.Description = \"Open GL Queue Explorer\"",
        "lnk.Save",
        "WScript.Echo \"Created Desktop shortcut: \" & fso.BuildPath(desktop, \"GL Queue Explorer.lnk\")",
        "",
    ]
    return "\r\n".join(lines)


def taskbar_ps1_text(html_name: str = HTML_NAME) -> str:
    """The taskbar-identity stamper written beside the page.

    Waits briefly for the Explorer's Edge/Chrome app window (exact title
    match on a Chrome_WidgetWin window — a browser TAB has a ' - Microsoft
    Edge' suffix and is never touched), then sets the window's
    System.AppUserModel properties: ID, RelaunchCommand,
    RelaunchDisplayNameResource and RelaunchIconResource. Windows then shows
    the fan icon on the taskbar, ungrouped from Edge, and pinning the button
    relaunches through the silent VBS opener. Pure ASCII + CRLF; run hidden
    by the .bat, the VBS opener, and the launcher's Open button."""
    del html_name  # the window title is the page <title>, a build constant
    csharp = [
        "using System;",
        ...
        "    void GetDescription([Out, MarshalAs(UnmanagedType.LPWStr)] StringBuilder d, int cch);",
        "    void SetDescription([MarshalAs(UnmanagedType.LPWStr)] string d);",
        ...
        "    void GetIconLocation([Out, MarshalAs(UnmanagedType.LPWStr)] StringBuilder p, int cch, out int idx);",
        "    void SetIconLocation([MarshalAs(UnmanagedType.LPWStr)] string p, int idx);",
        ...
        "",
        "    public static List<long> FindAppWindows(string title) {",
        ...
        "    }",
        "",
        "    public static bool Stamp(long hwnd, string appId, string relaunch,",
        "                             string name, string icon) {",
        "      Guid iid = typeof(IPropertyStore).GUID;",
        "      IPropertyStore store;",
        "      if (SHGetPropertyStoreForWindow(new IntPtr(hwnd), ref iid, out store) != 0)",
        "        return false;",
        "      try {",
        "        Set(store, 5, appId);      // System.AppUserModel.ID",
        "        Set(store, 2, relaunch);   // ...RelaunchCommand",
        "        Set(store, 4, name);       // ...RelaunchDisplayNameResource",
        "        Set(store, 3, icon);       // ...RelaunchIconResource",
        "        store.Commit();",
        "        return true;",
        ...
        "    }",
        "",
        "    public static void WriteShortcut(string lnkPath, string target,",
        "        string args, string workdir, string iconPath, string appId,",
        "        string description) {",
    ...
    lines = [
        "# glq_taskbar.ps1 - auto-generated by order_explorer.py next to the page.",
        "# Gives the Explorer's Edge/Chrome app window its own Windows taskbar",
        "# identity (AppUserModelID + relaunch name/command/icon), so the taskbar",
        "# shows the fan icon instead of the browser's - and pinning keeps it.",
        "# Safe to run any time; it waits briefly for the window, then stamps it.",
        '$ErrorActionPreference = "SilentlyContinue"',
        "$here = Split-Path -Parent $MyInvocation.MyCommand.Path",
        '$title = "GL Queue Explorer"',
        f'$appId = "{APP_USER_MODEL_ID}"',
        "# The shell loads taskbar/relaunch icons lazily and refuses network",
        "# paths (the icon then decays to a blank page after a few seconds),",
        "# and launching scripts straight off the share is slow (they get",
        "# security-scanned every time). Keep per-user local copies of the",
        "# icon and the silent opener and reference only those.",
        '$localDir = Join-Path $env:LOCALAPPDATA "GL Queue Explorer"',
        "New-Item -ItemType Directory -Force -Path $localDir | Out-Null",
        f'$srcIcon = Join-Path $here "{ICON_ICO_NAME}"',
        f'$localIcon = Join-Path $localDir "{ICON_ICO_NAME}"',
        "if (Test-Path $srcIcon) { Copy-Item -Force $srcIcon $localIcon }",
        "if (-not (Test-Path $localIcon)) { $localIcon = $srcIcon }",
        f'$srcVbs = Join-Path $here "{LAUNCH_VBS_NAME}"',
        f'$localVbs = Join-Path $localDir "{LAUNCH_VBS_NAME}"',
        "if (Test-Path $srcVbs) { Copy-Item -Force $srcVbs $localVbs }",
        "if (-not (Test-Path $localVbs)) { $localVbs = $srcVbs }",
        '$icon = $localIcon + ",0"',
        "$relaunch = 'wscript.exe \"' + $localVbs + '\"'",
        "",
        "$src = @'",
        *csharp,
        "'@",
        "# Compile once into a per-user cached DLL: Add-Type recompiles C# on",
        "# every run otherwise, which made opening noticeably slower.",
        '$dllRev = "1"',
        '$dll = Join-Path $localDir ("GLQTaskbar." + $dllRev + ".dll")',
        'if (-not ("GLQ.Taskbar" -as [type])) {',
        "  if (-not (Test-Path $dll)) {",
        '    Get-ChildItem -Path (Join-Path $localDir "GLQTaskbar.*.dll") |',
        "      Remove-Item -Force",
        "    Add-Type -TypeDefinition $src -OutputAssembly $dll",
        "  }",
        "  if (Test-Path $dll) { Add-Type -Path $dll }",
        '  if (-not ("GLQ.Taskbar" -as [type])) { Add-Type -TypeDefinition $src }',
        "}",
        "",
        "# The Start Menu shortcut is the canonical icon source for an",
        "# AppUserModelID: the taskbar resolves a group's icon from a Start",
        "# Menu .lnk whose ID matches before anything else, and pinning the",
        "# running window inherits it. Refresh it on every open.",
        '$startMenu = Join-Path $env:APPDATA "Microsoft\\Windows\\Start Menu\\Programs"',
        '$lnk = Join-Path $startMenu "GL Queue Explorer.lnk"',
        "[GLQ.Taskbar]::WriteShortcut($lnk, \"wscript.exe\", '\"' + $localVbs + '\"',",
        '  $localDir, $localIcon, $appId, "Open GL Queue Explorer")',
        "",
        "$stamped = @{}",
        "$deadline = (Get-Date).AddSeconds(25)",
        "while ((Get-Date) -lt $deadline) {",
        "  foreach ($h in [GLQ.Taskbar]::FindAppWindows($title)) {",
        "    if ([GLQ.Taskbar]::Stamp($h, $appId, $relaunch, $title, $icon)) {",
        '      $stamped["$h"] = $true',
        "    }",
        "  }",
        "  if ($stamped.Count -gt 0) { break }",
        "  Start-Sleep -Milliseconds 500",
        "}",
        "# Chromium can swap in the real frame window moments after it first",
        "# appears; one late pass restamps whatever holds the title now.",
        "Start-Sleep -Milliseconds 2000",
        "foreach ($h in [GLQ.Taskbar]::FindAppWindows($title)) {",
        "  [GLQ.Taskbar]::Stamp($h, $appId, $relaunch, $title, $icon) | Out-Null",
        "}",
        "",
    ]
    return "\r\n".join(lines)
...
def launch_vbs_text(html_name: str = HTML_NAME,
                    page_path: Path | str | None = None) -> str:
    """The silent app-window opener written beside the page: starts the
    Edge/Chrome app window (same fallback chain as the .bat, no console
    flash) and runs the taskbar-identity stamper. The taskbar helper also
    copies it to %LOCALAPPDATA% and points pins/relaunches at that copy, so
    the published absolute page path comes first and the beside-me lookup is
    the fallback for the copy sitting next to the page itself. Pure ASCII +
    CRLF."""
    abs_page = str(page_path) if page_path else ""
    lines = [
        "' Opens GL Queue Explorer as a clean app window and gives it its own",
        "' taskbar icon (see glq_taskbar.ps1). Auto-generated by order_explorer.py.",
        "Option Explicit",
        "Dim fso, shell, here, page, url, edge, chrome, exe, args, ps",
        'Set fso = CreateObject("Scripting.FileSystemObject")',
        'Set shell = CreateObject("WScript.Shell")',
        "here = fso.GetParentFolderName(WScript.ScriptFullName)",
        (f'page = "{abs_page}"' if abs_page else 'page = ""'),
        "If Not fso.FileExists(page) Then page = fso.BuildPath(here, "
        f'"{html_name}")',
        "If Not fso.FileExists(page) Then",
        f'  MsgBox "Could not find {html_name} next to this launcher.", 48, "GL Queue Explorer"',
        "  WScript.Quit 1",
        "End If",
        ...
        '  exe = "msedge"',
        "End If",
        'args = "--start-maximized --window-position=0,0 --window-size=3840,2160 --app=""" & url & """"',
        'shell.Run """" & exe & """ " & args, 1, False',
        "' The stamper lives beside the page; when this script runs from its",
        "' %LOCALAPPDATA% copy, look for it next to the page instead.",
        f'ps = fso.BuildPath(here, "{TASKBAR_PS1_NAME}")',
        "If Not fso.FileExists(ps) Then ps = fso.BuildPath(fso.GetParentFolderName(page), "
        f'"{TASKBAR_PS1_NAME}")',
...
def bat_text(html_name: str = HTML_NAME) -> str:
    ...
    lines = [
        ...
        ")",
        "",
        "rem The taskbar helper waits for the app window and gives it the fan",
        "rem icon (its own AppUserModelID) instead of grouping under Edge.",
        f'if exist "%~dp0{TASKBAR_PS1_NAME}" start "" /b powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0{TASKBAR_PS1_NAME}"',
        "",
...
def write_explorer(payload: Dict[str, Any], out: Path | None = None) -> Path:
    ...
    for target_name, data in (
        (ICON_PNG_NAME, explorer_icon.png_bytes()),
        (ICON_ICO_NAME, explorer_icon.ico_bytes()),
    ):
        try:
    ...
    try:
        if (not shortcut_vbs.exists()
                or shortcut_vbs.read_text(encoding="ascii", errors="replace") != shortcut_text):
            shortcut_vbs.write_bytes(shortcut_text.encode("ascii"))
    except OSError as e:
        log.warning("Could not write %s (%s)", shortcut_vbs, e)
    ...
    for helper_name, helper_text in (
        (LAUNCH_VBS_NAME, launch_vbs_text(out.name, page_path=out)),
        (TASKBAR_PS1_NAME, taskbar_ps1_text(out.name)),
    ):
        helper = out.parent / helper_name
        try:
            if (not helper.exists()
                    or helper.read_text(encoding="ascii", errors="replace") != helper_text):
                helper.write_bytes(helper_text.encode("ascii"))
        except OSError as e:
            log.warning("Could not write %s (%s)", helper, e)
...
def _stamp_taskbar_identity(page_dir: Path) -> None:
    """Run the published taskbar stamper hidden (best-effort): it waits for
    the app window and gives it its own AppUserModelID + fan icon, so the
    taskbar button stops showing Edge's icon. See taskbar_ps1_text()."""
    import os
    import subprocess
    if os.name != "nt":
        return
    ps1 = page_dir / TASKBAR_PS1_NAME
    if not ps1.exists():
        return
    try:
        subprocess.Popen(
        ...
        log.warning("Could not run %s (%s)", ps1, e)
...
def open_in_app_window(page: Path) -> bool:
    ...
    for env, sub in candidates:
        ...
        if exe and exe.exists():
            ...
            _stamp_taskbar_identity(page.parent)
            return True
    try:
